    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:22 }}>
          <div>
            <h2 style={{ fontSize:17, fontWeight:800 }}>{editData ? '✏️ Chỉnh sửa kế hoạch' : '🎯 Tạo kế hoạch tiết kiệm'}</h2>
            <p style={{ fontSize:12, color:'var(--text-muted)', marginTop:2 }}>Đặt mục tiêu và theo dõi tiến độ</p>
          </div>
          <button onClick={onClose} style={{ background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'var(--text-muted)', width:32, height:32, borderRadius:8, fontSize:16, cursor:'pointer' }}>✕</button>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <div>
            ...
            <input value={name} onChange={e=>setName(e.target.value)} placeholder="VD: Đi Nhật Bản, Mua MacBook..." style={inp} required />
          </div>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
              {PLAN_ICONS.map(ic => (
                <button key={ic} type="button" onClick={()=>setIcon(ic)} style={{
                  width:36, height:36, borderRadius:9, border:`1.5px solid ${icon===ic ? color : 'rgba(255,255,255,0.1)'}`,
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {PLAN_COLORS.map(c => (
                <button key={c} type="button" onClick={()=>setColor(c)} style={{
                  width:26, height:26, borderRadius:'50%', background:c, border:`3px solid ${color===c ? '#fff' : 'transparent'}`,
                  cursor:'pointer', transition:'all 0.15s', transform: color===c ? 'scale(1.2)' : 'scale(1)',
          ...
          <div style={{ display:'flex', gap:10 }}>
            <div style={{ flex:1 }}>
              ...
              <input value={fmt(target)} onChange={e=>setTarget(e.target.value)} placeholder="10.000.000" style={inp} inputMode="numeric" required />
            </div>
            <div style={{ flex:1 }}>
          ...
          {monthlyNeeded !== null && (
            <div style={{
              ...
              <div style={{ fontSize:13, fontWeight:700, marginBottom:4, color: feasible > 50 ? '#f87171' : feasible > 30 ? '#fbbf24' : '#34d399' }}>
                {feasible > 50 ? '⚠️ Khó khả thi' : feasible > 30 ? '🟡 Cần cố gắng' : '✅ Khả thi'}
              </div>
              <div style={{ fontSize:12, color:'var(--text-muted)', lineHeight:1.6 }}>
                Cần tiết kiệm <strong style={{ color:'#f1f5f9' }}>{formatCurrency(monthlyNeeded)}/tháng</strong> trong <strong style={{ color:'#f1f5f9' }}>{monthsLeft} tháng</strong>
                {avgMonthlyIncome > 0 && feasible !== null && (
                  <span> — chiếm <strong style={{ color: feasible > 50 ? '#f87171' : '#fbbf24' }}>{feasible.toFixed(0)}%</strong> thu nhập</span>
                )}
              </div>
            </div>
          )}
          ...
          <button
            onClick={() => {
              ...
              onSave({ name, icon, targetAmount: targetNum, savedAmount: savedNum, deadline, color, note })
              onClose()
            }}
            style={{
    ...
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', backdropFilter:'blur(6px)', zIndex:1002, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <h3 style={{ fontSize:16, fontWeight:800, marginBottom:4 }}>
          {plan.icon} Nạp tiền tiết kiệm
        </h3>
        <p style={{ fontSize:12, color:'var(--text-muted)', marginBottom:18 }}>{plan.name}</p>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={onClose} style={{ flex:1, padding:'11px 0', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:11, color:'var(--text-muted)', fontSize:14, cursor:'pointer' }}>Huỷ</button>
          <button
            onClick={() => { if (num > 0) { onAdd(plan.id, num); onClose() } }}
            style={{ flex:2, padding:'11px 0', background:'linear-gradient(135deg, #6366f1, #4f46e5)', border:'none', borderRadius:11, color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', boxShadow:'0 4px 16px rgba(99,102,241,0.4)' }}
          >+ Nạp {num ? formatCurrency(num) : 'tiền'}</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div
      ...
      style={{
        ...
        position:'relative', overflow:'hidden',
        animation:'fadeInUp 0.5s cubic-bezier(0.34,1.1,0.64,1) both',
      }}
    >
      {/* BG glow */}
      ...
      {/* Confetti if done */}
      {done && (
        <div style={{ position:'absolute', top:8, right:8, fontSize:20, animation:'float 2s ease-in-out infinite' }}>🎉</div>
      )}
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{
              ...
              fontSize:22, transition:'transform 0.3s ease',
              transform: hovered ? 'scale(1.15) rotate(5deg)' : 'scale(1)',
            }}>{plan.icon}</div>
            <div>
              <div style={{ fontWeight:700, fontSize:14, color:'var(--text)' }}>{plan.name}</div>
              {plan.note && <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:1 }}>{plan.note}</div>}
            </div>
          </div>
          <div style={{ display:'flex', gap:5 }}>
            <button onClick={() => onAddSaving(plan)} style={{ background:`${plan.color}20`, border:`1px solid ${plan.color}40`, borderRadius:8, padding:'5px 10px', color:plan.color, fontSize:12, fontWeight:700, cursor:'pointer', transition:'all 0.15s' }}>+ Nạp</button>
            <button onClick={() => onEdit(plan)} style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:7, width:28, height:28, color:'var(--text-muted)', fontSize:13, cursor:'pointer' }}>✏️</button>
            <button onClick={() => onDelete(plan.id)} style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:7, width:28, height:28, color:'#f87171', fontSize:13, cursor:'pointer' }}>🗑️</button>
          </div>
        </div>
        ...
        <div style={{ marginBottom:10 }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
            <span style={{ fontSize:13, color:'var(--text-muted)' }}>
              ...
              <span style={{ color:'var(--text-dim)' }}> / {formatCurrency(plan.targetAmount)}</span>
            </span>
          ...
          <div style={{ background:'rgba(255,255,255,0.07)', borderRadius:99, height:8, overflow:'hidden' }}>
            <div style={{
              ...
              background: done
                ...
                : `linear-gradient(90deg, ${plan.color}99, ${plan.color})`,
              boxShadow: `0 0 12px ${plan.color}80`,
              transition:'width 1.2s cubic-bezier(0.34,1.05,0.64,1)',
              position:'relative', overflow:'hidden',
            }}>
              {/* Shimmer on progress */}
              <div style={{
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          ...
          {monthlyNeeded > 0 && !done && (
            <div style={{ background:'rgba(255,255,255,0.04)', borderRadius:8, padding:'5px 10px', fontSize:11 }}>
              <span style={{ color:'var(--text-muted)' }}>Cần </span>
...
const lbl = { display:'block', fontSize:11, fontWeight:700, color:'var(--text-muted)', marginBottom:6, letterSpacing:'0.06em', textTransform:'uppercase' }
const inp = { width:'100%', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:11, padding:'11px 14px', color:'#f0f6ff', fontSize:14, outline:'none', fontFamily:'Inter,sans-serif', transition:'border-color 0.2s, box-shadow 0.2s' }
...
export default function SavingsPlans({ plans, onAdd, onUpdate, onDelete, onAddSaving, avgMonthlyIncome }) {
  ...
  const handleSave = (data) => {
    if (editPlan) onUpdate(editPlan.id, data)
    else onAdd(data)
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
        <div>
          <h2 style={{ fontSize:20, fontWeight:800, letterSpacing:'-0.3px', marginBottom:3,
            background:'linear-gradient(135deg, #f0f6ff, #818cf8)',
            WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent',
          }}>🎯 Kế hoạch tiết kiệm</h2>
          <p style={{ fontSize:12, color:'var(--text-muted)' }}>Đặt mục tiêu và theo dõi tiến độ tài chính</p>
        </div>
        <button
          ...
          onMouseOver={e => { e.currentTarget.style.transform='translateY(-1px) scale(1.03)'; e.currentTarget.style.boxShadow='0 6px 24px rgba(99,102,241,0.6)' }}
          onMouseOut={e => { e.currentTarget.style.transform='scale(1)'; e.currentTarget.style.boxShadow='0 4px 16px rgba(99,102,241,0.4)' }}
        >＋ Tạo kế hoạch</button>
      </div>

      {/* Overall progress (if has plans) */}
      {plans.length > 0 && (
        <div className="glass-card" style={{ padding:'16px 20px', marginBottom:16, animation:'slideDown 0.4s ease both' }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
            <span style={{ fontSize:13, color:'var(--text-muted)', fontWeight:500 }}>
              Tổng tiến độ — {plans.length} kế hoạch
            </span>
      ...
      {plans.length === 0 ? (
        <div className="glass-card" style={{ textAlign:'center', padding:'52px 24px' }}>
          ...
          <div style={{ fontSize:16, fontWeight:700, marginBottom:6 }}>Chưa có kế hoạch nào</div>
          <div style={{ fontSize:13, color:'var(--text-muted)', marginBottom:20 }}>
            Tạo kế hoạch đầu tiên để bắt đầu tiết kiệm
          </div>
          <button
            onClick={() => setModalOpen(true)}
            style={{ background:'linear-gradient(135deg,#6366f1,#4f46e5)', color:'#fff', border:'none', borderRadius:11, padding:'10px 24px', fontSize:14, fontWeight:700, cursor:'pointer', boxShadow:'0 4px 16px rgba(99,102,241,0.4)' }}
          >🚀 Tạo kế hoạch đầu tiên</button>
        </div>
      ) : (
          ...
          {plans.map((p, i) => (
            <div key={p.id} style={{ animationDelay:`${i*80}ms` }}>
              <PlanCard
                ...
                onEdit={handleEdit}
                onDelete={onDelete}
                onAddSaving={(plan) => setSavingModal(plan)}
              />
            </div>
          ))}
        </div>
