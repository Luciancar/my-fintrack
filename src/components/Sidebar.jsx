...
export default function Sidebar({ activeTab, onTabChange, collapsed, onToggleCollapse }) {
  ...
  return (
    <aside style={{
      ...
      background: 'var(--header-bg)', backdropFilter: 'blur(32px)', borderRight: '1px solid var(--border)',
      display: 'flex', flexDirection: 'column', transition: 'width 0.2s ease, background 0.3s ease, border-color 0.3s ease',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: collapsed ? '20px 0' : '20px 18px', justifyContent: collapsed ? 'center' : 'flex-start' }}>
        ...
        {!collapsed && (
          <div>
            <div style={{ fontWeight: 900, fontSize: 16, color: 'var(--text)', letterSpacing: '-0.5px', lineHeight: 1 }}>FinTrack</div>
            <div style={{ fontSize: 9, color: 'var(--text-label)', letterSpacing: '1.5px', marginTop: 2 }}>QUẢN LÝ CHI TIÊU</div>
          </div>
        )}
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: active ? 'var(--accent-soft)' : 'transparent', border: 'none', borderRadius: 10,
                color: active ? 'var(--accent-text)' : 'var(--text-dim)', cursor: 'pointer', fontSize: 14,
                fontWeight: active ? 700 : 500, position: 'relative', transition: 'background 0.15s, color 0.3s',
              }}
            >
              <span style={{ fontSize: 17 }}>{tab.icon}</span>
              {!collapsed && <span>{tab.label}</span>}
              {active && (
                <span style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', width: 3, height: 18, borderRadius: 2, background: 'var(--accent-text)' }} />
              )}
            </button>
          )
        })}
      ...
      <div style={{ padding: 12, borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <button
          ...
          style={{
            ...
            color: 'var(--text-label)', cursor: 'pointer', fontSize: 13,
          }}
        >
